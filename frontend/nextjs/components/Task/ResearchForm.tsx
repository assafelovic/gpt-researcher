...
interface ResearchFormProps {
  ...
  onFormSubmit?: (task: string, reportType: string, reportSource: string) => void;
...
export default function ResearchForm({
  ...
  useEffect(() => {
    // Optional: If needed, set default report type when form loads
    setChatBoxSettings((prevSettings: any) => ({
      ...prevSettings,
      report_type: defaultReportType,
    }));
  }, [defaultReportType, setChatBoxSettings]);

  return (
    <form method="POST" className="mt-3 report_settings" onSubmit={handleSubmit}>
