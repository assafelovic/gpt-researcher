...
export default function MobileLayout({
  ...
  const formatTimestamp = (timestamp: number | string | Date | undefined) => {
    ...
    try {
      ...
      if (isNaN(date.getTime())) return 'Unbekannte Zeit';
    ...
    <main className="flex flex-col min-h-screen bg-gray-900">
        ...
        {showHistory && (
          <div className="px-4 py-3 bg-gray-800/90 border-t border-gray-700/50 animate-slide-down shadow-lg max-h-[70vh] overflow-y-auto custom-scrollbar">
            <div className="mb-3 flex justify-between items-center">
              <h3 className="text-sm font-medium text-gray-200">Rechercheverlauf</h3>
              <button
            ...
            {history.length > 0 ? (
              ...
              <div className="text-center py-6">
                ...
                <p className="text-sm text-gray-400">Noch kein Rechercheverlauf vorhanden</p>
                <button
                  onClick={onNewResearch}
            ...
            {history.length > 0 && (
              <div className="mt-3 pt-2 border-t border-gray-700/30 flex justify-center">
                ...
                >
                  Gesamten Rechercheverlauf anzeigen
                </a>
              </div>
            )}
          </div>
        ...
        {showSettings && (
          <div className="px-4 py-3 bg-gray-800/90 border-t border-gray-700/50 animate-slide-down shadow-lg">
            <div className="mb-2 flex justify-between items-center">
              <h3 className="text-sm font-medium text-gray-200">Einstellungen</h3>
              <button
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-xs text-gray-400 mb-1">Berichtsart</label>
                <select
                ...
                >
                  <option value="research_report">Kurzfassung - Schnell und knapp (~2 Min.)</option>
                  <option value="deep">Tiefenrecherche-Bericht</option>
                  <option value="multi_agents">Multi-Agenten-Bericht</option>
                  <option value="detailed_report">Ausführlich - Detaillierter und länger (~5 Min.)</option>
                </select>
              </div>

              <div>
                <label className="block text-xs text-gray-400 mb-1">Quellenmodus</label>
                <select
                  className="w-full bg-gray-900 border border-gray-700 rounded-md py-1.5 px-2 text-sm text-gray-300 focus:outline-none focus:ring-1 focus:ring-teal-500 focus:border-teal-500"
                ...
                {chatBoxSettings.report_source === "local" || chatBoxSettings.report_source === "hybrid" ? (
                  <div className="mt-2 rounded-lg border border-gray-700/50 bg-gray-900/60 p-2">
                    <FileUpload />
                  </div>
                ) : null}
                {chatBoxSettings.report_source === "onion" ? (
              ...
              <div>
                <label className="block text-xs text-gray-400 mb-1">Recherche-Ton</label>
                <select
                  className="w-full bg-gray-900 border border-gray-700 rounded-md py-1.5 px-2 text-sm text-gray-300 focus:outline-none focus:ring-1 focus:ring-teal-500 focus:border-teal-500"
                  value={chatBoxSettings.tone}
                  onChange={(e) => setChatBoxSettings({...chatBoxSettings, tone: e.target.value})}
                >
                  <option value="Objective">Objektiv - Unparteiische und ausgewogene Darstellung von Fakten</option>
                  <option value="Formal">Formal - Entspricht akademischen Standards</option>
                  <option value="Analytical">Analytisch - Kritische Auswertung von Daten</option>
                  <option value="Persuasive">Überzeugend - Eine klare Perspektive vermitteln</option>
                  <option value="Informative">Informativ - Klare und umfassende Informationen</option>
                  <option value="Simple">Einfach - Einfache Sprache und klare Erklärungen</option>
                  <option value="Casual">Locker - Gesprächiger Stil</option>
                </select>
              </div>
              ...
              <div>
                ...
                >
                  <option value="copilot">Copilot - Chat-Ansicht</option>
                  <option value="document">Dokument - Klassisches Berichts-Layout</option>
                </select>
              </div>
            </div>
          </div>
      ...
      <footer className="mt-auto py-3 px-4 text-center border-t border-gray-800/40 bg-gray-900/80 backdrop-blur-sm">
        ...
        <div className="text-xs text-gray-400">
          © {new Date().getFullYear()} GPT Researcher. Alle Rechte vorbehalten.
        </div>
      </footer>
