...
class WebSocketManager:
    ...
    async def start_streaming(self, task, report_type, websocket, rabbit):
        """Start streaming the output."""
        report = await run_agent(task, report_type, websocket, rabbit)
        return report


async def run_agent(task, report_type, websocket, rabbit):
    """Run the agent."""
    # measure time
    ...
    if report_type == ReportType.DetailedReport.value:
        researcher = DetailedReport(query=task, source_urls=None, config_path=config_path, websocket=websocket, rabbit=rabbit)
    else:
        researcher = BasicReport(query=task, report_type=report_type,
                                 source_urls=None, config_path=config_path, websocket=websocket, rabbit=rabbit)
