...
class GPTResearcher:
    ...
    def _process_mcp_configs(self, mcp_configs: list[dict]) -> None:
        ...
        retriever_set = set(self.headers.get("retrievers", "").split(",")) if self.headers.get("retrievers") else set()
        if "mcp" not in retriever_set and retriever_set:
            retriever_set.add("mcp")
            self.headers["retrievers"] = ",".join(filter(None, retriever_set))
        elif not retriever_set:
            self.headers["retriever"] = "mcp"
        
        # Process each MCP configuration
        for i, config in enumerate(mcp_configs):
            # For multiple MCP configs, use prefix mcp1_, mcp2_, etc.
            prefix = "" if len(mcp_configs) == 1 else f"mcp{i+1}_"
            ...
            for key, value in config.items():
                if key == "env" and isinstance(value, dict):
                    ...
                    for env_key, env_value in value.items():
                        self.headers[f"{prefix}mcp_env_{env_key}"] = env_value
                elif key == "server_args":
                    # Handle server_args as either list or string
                    if isinstance(value, list):
                        self.headers[f"{prefix}mcp_server_args"] = " ".join(value)
                    else:
                        self.headers[f"{prefix}mcp_server_args"] = str(value)
                # Skip tool_args as we want arguments to be dynamically generated by the LLM
                elif key != "tool_args":
                    # Standard parameters
                    self.headers[f"{prefix}mcp_{key}"] = str(value)
