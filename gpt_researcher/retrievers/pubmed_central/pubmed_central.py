...
class PubMedCentralSearch:
    ...
    def _retrieve_api_key(self) -> str:
        ...
        """
        try:
            api_key: str = os.environ["NCBI_API_KEY"]
        except KeyError:
            raise Exception("NCBI API key not found. Please set the NCBI_API_KEY environment variable. " "You can obtain your key from https://www.ncbi.nlm.nih.gov/account/")
        return api_key
    ...
    def search(self, max_results: int = 10) -> list[dict[str, Any]]:
        ...
        for article_id in ids:
            ...
            if self.has_body_content(xml_content):
                article_data: dict[str, Any] = self.parse_xml(xml_content)
                if article_data:
    ...
    def parse_xml(self, xml_content: str) -> dict[str, Any] | None:
