...
def get_prompt_by_report_type(
    ...
    """Factory function to generate the prompt based on the given report type
    and prompt family.

    Args:
        report_type (str): The report type to generate the prompt for.
        prompt_family (type[PromptFamily] | PromptFamily): The prompt family to
            use. If passed as a type, methods must be static
    """
    prompt_by_type = getattr(prompt_family, _report_type_mapping.get(report_type, ""), None)
    default_report_type = ReportType.ResearchReport.value
    if not prompt_by_type:
        warnings.warn(
            f"Invalid report type: {report_type}.\n"
            f"Please use one of the following: {', '.join([enum_value for enum_value in _report_type_mapping.keys()])}\n"
...
_report_type_mapping = {
    ...
    # TODO: This seems to be unused and the function signature doesn't match the
