...
class ResearchConductor:
    ...
    def _is_onion_mode(self) -> bool:
        return self.researcher.report_source == ReportSource.Onion.value

    def _resolve_proxy_url(self) -> str | None:
        if not self._is_onion_mode():
            return None
        return resolve_onion_proxy_url(self.researcher.cfg)

    def _primary_retriever_class(self):
        if self._is_onion_mode():
            return Duckduckgo
        if self.researcher.retrievers:
            return self.researcher.retrievers[0]
        return Duckduckgo
    ...
    async def plan_research(self, query, query_domains=None):
        ...
        if self._is_onion_mode():
            await stream_output(
                "logs",
                "planning_research",
                f"🕵️ Browsing the onion/Tor network to learn more about the task: {query}...",
    ...
    async def conduct_research(self):
        ...
        onion_proxy_url = self._resolve_proxy_url()
        if self._is_onion_mode() and not onion_proxy_url:
            self.logger.warning(
                "Onion research mode selected without ONION_PROXY_URL; falling back to direct HTTP requests."
            )
        ...
        has_mcp_retriever = (
            not self._is_onion_mode()
            and any("mcpretriever" in r.__name__.lower() for r in self.researcher.retrievers)
        )
        ...
        elif self.researcher.report_source in (ReportSource.Web.value, ReportSource.Onion.value):
    ...
    async def _get_context_by_urls(self, urls, proxy_url: str | None = None):
    ...
    async def _get_context_by_web_search(
        ...
        mcp_retrievers = [] if self._is_onion_mode() else [
        ...
        try:
            context = await asyncio.gather(
                *[
                    self._process_sub_query(sub_query, scraped_data, query_domains, proxy_url=proxy_url)
                    for sub_query in sub_queries
                ]
            )
    ...
    async def _process_sub_query(
        ...
        try:
            ...
            if not scraped_data:
                scraped_data = await self._scrape_data_by_urls(sub_query, query_domains, proxy_url=proxy_url)
    ...
    def _combine_mcp_and_web_context(self, mcp_context: list, web_context: str, sub_query: str) -> str:
        ...
        if mcp_context:
            ...
            for i, item in enumerate(mcp_context):
                ...
                normalized_title = normalize_source_title(title, url)
                ...
                if content and content.strip():
                    ...
                    formatted_content = f"{content_text}{citation}"
    ...
    async def _search_relevant_source_urls(
        ...
        retriever_classes = [Duckduckgo] if self._is_onion_mode() else self.researcher.retrievers
        for retriever_class in retriever_classes:
            ...
            try:
                ...
                if retriever_class.__name__.lower() == "duckduckgo" and proxy_url:
