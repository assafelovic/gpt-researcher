...
#         # Remove it and call methdod directly from Global data API module
...
#                     api_data[function_name] = {'error': str(e)}
#         return api_data
...
#         # Fetch API data
#         api_data = await self.fetch_api_data(section_data)
...
#         # Set document path for local research
#         researcher.cfg.doc_path = self.doc_path
        
#         # Set report source to hybrid (web + local docs)
#         researcher.cfg.report_source = "hybrid"
...
#                 'sources': researcher.visited_urls if hasattr(researcher, 'visited_urls') else [],
#                 'timestamp': datetime.now().isoformat()
#             }
            
#             logger.info(f"Completed research for section: {section_name}")
...
#             logger.info(f"Processing section: {section_name} (relevance: {section_data.get('relevance_score', 0)})")            
...
#     print(f"Sections processed: {len(results['section_results'])}")
#     print(f"Timestamp: {results['timestamp']}")
#     print("\nSection Results:")
#     # for section_name, section_data in results['section_results'].items():
#     #     status = "✓ Success" if 'error' not in section_data else "✗ Error"
...
class AbbVieResearchPipeline:
    ...
    async def conduct_section_research(self, section_name: str, section_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'data' not in api_result or 'summary' not in api_result:
        ...
        researcher = GPTResearcher(query=main_query, report_source="hybrid")
        try:
...
async def main():
    ...
    print("\nSection Results:")
    print(results)
    print("Pipeline completed successfully! GPT Researcher continued even when API calls failed.")
