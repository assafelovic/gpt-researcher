...
```mermaid
graph TD
    A[Research Task] --> B[Initialize GPT Researcher with mcp_configs]
    
    B --> C[Stage 1: Smart Tool Selection]
    C -->     D[GPTR analyzes query + available MCP servers]
    D --> E[Select 2-3 most relevant tools]
    
    E --> F[Stage 2: Contextual Research]
    F -->     G[GPTR uses selected tools with dynamic arguments]
    
    B --> H[Parallel Web Search]
    H --> I[Combine MCP + Web Results]
    G --> I
