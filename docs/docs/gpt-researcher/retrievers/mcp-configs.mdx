...
1. **Stage 1: Smart Tool Selection** - AI analyzes your query and available MCP servers to select the most relevant tools
2. **Stage 2: Contextual Research** - AI uses selected tools with dynamically generated, query-specific arguments
...
# Simple MCP configuration - works automatically with optimal settings
...
### Quantum Computing Research (Advanced Example)

Demonstrating specialized domain research with custom MCP servers:
...
researcher = GPTResearcher(
    ...
    mcp_strategy="deep",  # Run MCP for each sub-query
    mcp_configs=[...]
)
...
researcher = GPTResearcher(
    ...
    mcp_strategy="disabled"  # Skip MCP entirely
)
```
...
1. **Use descriptive server names** for easier debugging
...
5. **Combine MCP with web search** for comprehensive research
6. **Use multiple MCP servers** for complex research domains
7. **Let the default settings handle optimization** for most use cases
...
```python
# Old headers-based configuration (no longer supported)
researcher = GPTResearcher(
    query="your query",
    headers={
...
# New mcp_configs approach (current)
researcher = GPTResearcher(
    query="your query",
    mcp_configs=[
        {
