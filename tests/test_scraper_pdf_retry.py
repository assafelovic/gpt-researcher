...
class ExtractDataFromUrlPdfRetryTests(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        # extract_data_from_url runs the SSRF guard before scraping, and the
        # guard resolves the hostname. These tests use deliberately
        # unresolvable example hosts and are not exercising URL safety, so
        # neutralise the guard for the duration of the class.
        import gpt_researcher.scraper.scraper as scraper_module
        self._original_validate_url = scraper_module.validate_url
        scraper_module.validate_url = lambda url, **kwargs: url

    def tearDown(self):
        import gpt_researcher.scraper.scraper as scraper_module
        scraper_module.validate_url = self._original_validate_url
