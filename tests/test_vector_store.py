...
def load_document() -> list[Document]:
    document: list[Document] = [Document(page_content=essay)]
...
async def test_gpt_researcher_with_vector_store():
    docs: list[Document] = load_document()
    vectorstore: VectorStore = create_vectorstore(docs)
...
async def test_store_in_vector_store_web():
    vector_store: VectorStore = InMemoryVectorStore(embedding=OpenAIEmbeddings())
    query: str = "Which one is the best LLM"
    ...
    related_contexts: list[Document] = await vector_store.asimilarity_search("GPT-4o", k=2)
...
async def test_store_in_vector_store_urls():
    vector_store: VectorStore = InMemoryVectorStore(embedding=OpenAIEmbeddings())
    query: str = "Who won the world cup in 2022"
...
async def test_store_in_vector_store_langchain_docs():
    vector_store: VectorStore = InMemoryVectorStore(embedding=OpenAIEmbeddings())
    docs: list[Document] = load_document()
    query: str = "What does successful people tend to do?"
